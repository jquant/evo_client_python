...
class MemberProfile(BaseModel):
    ...
    def __init__(self, **data):
        ...
        now = datetime.now()
        active_contracts = [
            contract for contract in self.contracts_history
            if (contract.start_date and contract.start_date <= now and
                (not contract.end_date or contract.end_date > now) and
                contract.status == MembershipStatus.ACTIVE)
        ]
        self.current_contract = active_contracts[0] if active_contracts else None
        
        # Update activity summary
        self.total_entries = len(self.entries_history)
        self.total_classes_attended = len([a for a in self.activities_history if a.get('status') == 'attended'])
...
class MembersFiles(BaseModel):
    ...
    members: Dict[int, MemberProfile] = Field(
        ...
        description="Member profiles indexed by member ID"
    )
            ...
            if member.contracts_history:
                ...
                for contract in member.contracts_history:
                    if contract.start_date:
                        ...
                        months = (end_date - contract.start_date).days / Decimal('30.44')  # Average month length
