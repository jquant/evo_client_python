...
from evo_client.models.gym_model import GymOperatingData, GymEntry, MembershipStatus
...
class ChainAnalytics:
    ...
    def __init__(
        self,
        operating_data_list: List[GymOperatingData],
        previous_month_data: Optional[List[GymOperatingData]] = None,
    ):
        self.current_data = operating_data_list
        self.previous_data = previous_month_data or []

        # Basic metrics
        self.total_active_members = sum(
            data.total_active_members for data in operating_data_list
        )
        self.total_revenue = sum(data.mrr for data in operating_data_list)
        self.total_cross_branch_revenue = sum(
            data.cross_branch_revenue for data in operating_data_list
        )
    ...
    def _calculate_revenue_metrics(self) -> RevenueMetrics:
        ...
        previous_mrr = (
            sum(data.mrr for data in self.previous_data)
        ...
        churned_mrr = (
            sum(
                ...
                for contract in data.active_contracts
                if not any(
                    c.member_id == contract.member_id
                    for c in self.current_data[0].active_contracts
                )
            )
            if self.previous_data
            else Decimal("0")
                ...
                for curr_contract in curr_data.active_contracts:
                    prev_contract = next(
                        (
                            ...
                            if c.member_id == curr_contract.member_id
                        ),
                        None,
                    )
                    if prev_contract:
            ...
            for data in self.current_data:
                for contract in data.active_contracts:
                    ...
                    was_churned = any(
                        not any(
                            c.member_id == contract.member_id
                            and c.status == MembershipStatus.ACTIVE
                            for c in prev_data.active_contracts
                        )
        ...
        nrr = (
            Decimal(
                str(
                    (previous_mrr + expansion_mrr - contraction_mrr - churned_mrr)
                    / previous_mrr
                    * 100
                )
            )
    ...
    def _calculate_chain_metrics(self) -> ChainMetrics:
        ...
        acv = (
            Decimal(str(total_contract_value / total_contracts))
        ...
        arpu = (
            Decimal(str(self.revenue_metrics.mrr / self.total_active_members))
        ...
        denominator = (
            self.revenue_metrics.contraction_mrr + self.revenue_metrics.churned_mrr
        )
        quick_ratio = (
            Decimal(
                str(
                    (self.revenue_metrics.new_mrr + self.revenue_metrics.expansion_mrr)
                    / denominator
                )
            )
            if denominator > 0
        ...
        multi_unit_ratio = (
            Decimal(str(total_multi_unit_members))
            / Decimal(str(self.total_active_members))
            * Decimal("100")
            if self.total_active_members > 0
            else Decimal("0")
        ...
        cross_branch_ratio = (
            Decimal(str(len(self.cross_branch_entries)))
    ...
    def _rank_branches(
        self, operating_data_list: List[GymOperatingData]
    ) -> Dict[str, List[int]]:
        """Rank branches by different metrics."""
        metrics = {
            "revenue": [(i, data.mrr) for i, data in enumerate(operating_data_list)],
            "members": [
                (i, data.total_active_members)
                for i, data in enumerate(operating_data_list)
            ],
            "multi_unit": [
                (i, data.multi_unit_member_percentage)
                for i, data in enumerate(operating_data_list)
            ],
            "retention": [
                (i, 100 - data.churn_rate) for i, data in enumerate(operating_data_list)
            ],
        }
...
async def main():
    ...
    previous_data = await gym_api.get_operating_data(
        from_date=prev_start_date, to_date=prev_end_date
    )
